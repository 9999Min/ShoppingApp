...
import { getMemberOrderNowApi, getMemberOrderPreApi, postMemberOrderApi } from '@/services/order'
...
const getMemberOrderPreData = async () => {
  ...
  if (query.count && query.skuId) {
    const res = await getMemberOrderNowApi({ skuId: query.skuId, count: query.count })
...
const submitOrder = async () => {
  ...
  //提交成功后，关闭当前页面，跳转到订单详情 --使用redirectTo
...
<template>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: !defaultAddress?.id }" @tap="submitOrder">
      提交订单
    </view>
  </view>
</template>
