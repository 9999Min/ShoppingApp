...
//接受传递的type值，根据type值发送请求
const props = defineProps<{ orderState: number }>()

//定义数据，请求数据
const orderData = ref<OrderItem[]>()
const getOrderData = async () => {
  const res = await getMemberOrderAPI({ page: 1, pageSize: 10, orderState: props.orderState })
  orderData.value = res.result.items
}
onMounted(() => {
  getOrderData()
})
...
const toPayOrder = async (id: string) => {
  ...
  //更新订单状态
  const order = orderData.value?.find((v) => v.id === id)
    ...
    <view class="card" v-for="item in orderData" :key="item.id">
      ...
      <view class="action">
        ...
        <template v-if="item.orderState === OrderState.DaiFuKuan">
          <view class="button primary" @tap="toPayOrder(item.id)">去支付</view>
        </template>
        <template v-else>
          <navigator
