...
import { getCategoryPageDataApi } from '@/services/category'
...
//定义分类数据和获取数据的方法
const categoryData = ref<Array<categoryTopItem>>([])
const getCategoryData = async () => {
  const res = await getCategoryPageDataApi()
...
//二级商品分类的数据---计算属性
const categoryChildrenData = computed(() => {
  ...
  <view class="viewport" v-if="isLoaded">
    ...
    <view class="categories">
      ...
      <scroll-view class="secondary" scroll-y>
        <!-- 焦点图 -->
        <XtxSwiper class="banner" :list="swiperList" />
        <!-- 内容区域 -->
        <view class="panel" v-for="item in categoryChildrenData" :key="item.id">
          <view class="title">
            <text class="name">{{ item.name }}</text>
